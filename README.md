# Quick Launcher
[![Codacy Badge](https://app.codacy.com/project/badge/Grade/d7b711323b344cdab2e75b31890d950c)](https://www.codacy.com/gh/rodichenko/quick-launcher/dashboard?utm_source=github.com&amp;utm_medium=referral&amp;utm_content=rodichenko/quick-launcher&amp;utm_campaign=Badge_Grade)
...
* Optionally set the following env vars:
  ...
  * `USE_CLOUD_PIPELINE_API` - whether to use (`=1` or `=true`) Cloud Pipeline API for fetching configurations and launching the apps, default us `false`; if `USE_CLOUD_PIPELINE_API` is set to `true`, `CP_APPLICATIONS_API` will be treated as Cloud Pipeline API endpoint;
...
To build application that uses Cloud Pipeline API execute:
```shell script
npm install
npm run build-use-cp-api
```

Build assets will be placed to the **build** directory.
